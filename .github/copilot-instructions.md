...
`main` is protected: direct pushes are rejected, so the version bump, tag, and publish are three separate steps. Two packages publish to npm — **`quendor`** and **`zexplorer`** — each versioned, tagged, and published independently. `zexplorer` depends on `quendor` (a `workspace:*` that pnpm rewrites to the resolved version on publish), so when a `zexplorer` release needs a new engine API, publish `quendor` first. In the steps below `<pkg>` is the package (`quendor` or `zexplorer`) and `<dir>` is `packages/<pkg>`; each package ships only its build output via its `files` allowlist (`quendor` → `dist`, `zexplorer` → `dist-cli`, i.e. the `zexp` CLI, not the web app).

- [ ] **Bump (its own PR).** On a branch, from `<dir>`, run `vp run release --release X.Y.Z --yes`. Use `vp run` (not `pnpm run`), and pass the version explicitly: `bumpp`'s interactive prompt does not receive keystrokes through the `vp run` wrapper (the list renders, but the arrow keys are dead and it aborts), so `--release X.Y.Z --yes` is required to skip it. This commits the bump only (the script passes `--no-tag --no-push`); open a PR and merge it like any other change.
- [ ] **Tag the merged commit.** After the bump PR merges, on an up-to-date `main`, run `git tag <pkg>@X.Y.Z && git push origin <pkg>@X.Y.Z`. Tags are not covered by the branch ruleset, so this push is unaffected by it. Do not tag before the bump commit is merged — depending on the PR merge method, the commit that lands on `main` may not be the one `bumpp` created on the branch.
- [ ] **Publish (manual — no CI does it).** Raw `npm` is blocked by the repo's `devEngines.packageManager: pnpm`, so use pnpm. From `<dir>`: `pnpm login` if not already authenticated (`pnpm whoami` to check), then `pnpm publish --dry-run` to rehearse (builds via `prepublishOnly`, packs, no upload), then `pnpm publish` for the real, irreversible upload. `publishConfig.access` is `public`; have your npm OTP ready if 2FA is on (`pnpm publish --otp=<code>` if the prompt misbehaves).
- [ ] **(Optional) GitHub Release.** Cut one from the tag for public notes: `gh release create <pkg>@X.Y.Z --verify-tag --latest --title "..." --notes "..."`.
